            ...
            if (  map.attributes.markers && 0 < map.attributes.markers.length ) {
                map.attributes.markers.forEach( marker => {
                    ...
                    if ( marker.title || marker.description ) {
                        const contentString = `<div class="gutentor-map-overview"><h6 class="gutentor-map-overview-title">${ marker.title }</h6><div class="gutentor-map-overview-content">${ marker.description ? `<p>${ marker.description }</p>` : '' }</div></div>`;
